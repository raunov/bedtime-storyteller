...
if not openrouter_api_key:
    st.error("OPENROUTER_API_KEY is not set. Please add it to your environment variables or secrets.")
    st.stop()
...
if not selected_model:
    selected_model = random.choice(["openai/gpt-5", "anthropic/claude-sonnet-4", "google/gemini-2.5-pro", "x-ai/grok-4"])
...
def generate_story(children_info, story_details, language):
    ...
    try:
        ...
        logging.error(f"Error generating story with OpenRouter: {e}")
